...
React 클래스를 일반적인 JavaScript 클래스로 선언할 수도 있습니다. 다음의 예제는 ES6 class 문법을 사용합니다:
...
API는 `getInitialState`를 제외하고 `React.createClass`와 유사합니다. 별도의 `getInitialState` 메소드 대신에, 필요한 `state` 프로퍼티를 constructor에 설정할 수 있습니다.

또다른 차이점은 `propTypes`와 `defaultProps`가 클래스의 내부가 아니라 constructor에 프로퍼티로 정의되어 있다는 것입니다.
