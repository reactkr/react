...
천번째 인자는 객체(업데이트를 위한 key를 포함할 수 있습니다)이거나 업데이트를 위한 keys를 포함한 객체를 반환하는 함수(state와 props의)일 수 있습니다.
...
`function(state, props)`처럼 인자를 포함한 함수를 넘겨주는 것도 가능합니다. 어떤 값이든 state와 props의 이전 값을 참고해서 작은 업데이트를 하려는 경우 이는 유용합니다. 예를 들어 state의 값을 증가 시키려는 경우 같이 처리 가능합니다:
...
두번째 인자는 선택적이며, `setState`가 한번 완료되고 컴포넌트가 다시 렌더 되었을떄 실행되는 콜백 함수입니다.
